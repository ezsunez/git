...
namespace mini_download_new
{
    public partial class Form1 : Form
    {
        ...
        string zmlj = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory);
        string wllj = "本地连接";
        string wlqy = "启用";
        string wlty = "停用";
        string file = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory)+ @"\miniTEMP.txt";
        string fileyuan = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + @"\miniTEMP原文件.txt";
        ...
        private void  Form1_Load(object sender, EventArgs e)
        {
            ...
            foreach (NetworkInterface a in adapters)
            {
                if (a.Name == "本地连接")
                {
                    ...
                    RegistryKey software = hkml.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}");
                    subkeyNames = software.GetSubKeyNames();
                    foreach (string name in subkeyNames)
                    { try { 
                        string DDvalue = hkml.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}\" + name).GetValue("DriverDesc").ToString();
                            //MessageBox.Show(DDvalue.ToLower() + "!!!" + a.Description.ToLower());
                            if (a.Description.ToLower().IndexOf (DDvalue.ToLower())==0)
                        {
                            zcb = name;
                                //MessageBox.Show(zcb);
                            break;
